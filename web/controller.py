...
def dl_urlpost():
    ...
    if len(str.strip(youtube_url)) > 0:
       dlurl_task.delay(youtube_url,musictype)
       #return "<p>Download requested: "+ dlurl + "</p>"
       return template('dlform.tpl', musictypes = music_types.musictypes
                       , ret_string="Download requested: "+ youtube_url, commands=onkyo_command_list)
    else:
