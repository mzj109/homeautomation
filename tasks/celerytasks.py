...
# 
@celery_app.task(name='dlurl_task',bind=True, autoretry_for=(Exception,), exponential_backoff=25, retry_kwargs={'max_retries': 1}, retry_jitter=True)
def dlurl_task(self,youtube_url, musictype):
    subprocess.check_call(['bash','/home/volumio/code/shell/youtube_downloader.sh',youtube_url,musictype])
...
@celery_app.task(bind=True)
def volumio_task(self,the_command):
    print(the_command)
